        ...
        child: state.when(
          data: (PhysioState data) {
            final List<Exercise> planned = data.allExercises
                .where((Exercise item) =>
                    data.plannedExerciseIds.contains(item.id))
                .toList();
                ...
                if (planned.isEmpty)
                  const Center(
                    child: Padding(
                      ...
                      child: Text('No exercises in your plan yet.',
                          ...
                          children: <Widget>[
                            ...
                            if (isDone)
                              const Padding(
                                ...
                                child: Text('DONE ✓',
                                    style: TextStyle(
                                        color: Color(0xFFA3B19E),
                                        letterSpacing: 2.0)),
                              ),
                            const SizedBox(height: 16),
                            if ((data.planHistoryByExercise[exercise.id] ?? [])
                                .isNotEmpty) ...[
                              const SizedBox(height: 8),
                              const Divider(
                              ...
                              const SizedBox(height: 8),
                              ...((data.planHistoryByExercise[exercise.id] ??
                                      <PlanProgressEntry>[])
                                  .take(3)
                                  .map(
                                    (PlanProgressEntry entry) => Center(
                                      child: Text(
